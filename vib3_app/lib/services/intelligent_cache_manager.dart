...
class IntelligentCacheManager {
  ...
  static const int _maxDiskCacheMB = 1000;
  ...
  Future<void> _addToMemoryCache(String key, List<int> data) async {
    ...
    print('💾 Added to memory cache: $key');
  }
  ...
  void _evictFromMemoryCache() {
    ...
    if (oldestKey != null) {
      ...
      print('💾 Evicted from memory cache: $oldestKey');
    }
  }
  ...
  Future<void> _optimizeCache() async {
    ...
    if (hitRate < 0.5) {
      ...
      print('💾 Low hit rate - adjusting prediction algorithm');
    }
    ...
    await for (final entity in cacheDir.list()) {
      if (entity is File) {
        ...
        if (now.difference(stat.modified).inDays > 7) {
          await entity.delete();
          print('💾 Deleted old cache: ${entity.path}');
        }
      }
    }
  }
  ...
  Future<void> prefetchVideos(List<String> urls) async {
    ...
    for (int i = 0; i < urls.length && i < 3; i++) {
      ...
      if (priority > 0.6) {
        ...
        print('💾 Prefetching video ${i + 1} with priority: $priority');
      }
    }
  }
