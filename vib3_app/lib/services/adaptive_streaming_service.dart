...
class AdaptiveStreamingService {
  ...
  Future<void> initialize() async {
    print('🎬 Initializing Adaptive Streaming Service');
  ...
  Future<void> _detectDeviceCapabilities() async {
    try {
      if (Platform.isAndroid) {
        ...
        // Try to get RAM info (not always available)
        final memInfo = androidInfo.systemFeatures;
        print('📱 Android SDK: $sdkInt, Performance: $_devicePerformance');
        ...
        print('📱 iOS Model: $model, Performance: $_devicePerformance');
      }
    } catch (e) {
  ...
  void _updateNetworkQuality(List<ConnectivityResult> results) {
    ...
    if (newQuality != _currentNetworkQuality) {
      ...
      print('📶 Network quality changed to: $newQuality');
    }
  }
  ...
  Future<void> _estimateBandwidth() async {
    try {
      ...
      print('❌ Error estimating bandwidth: $e');
    }
  }
  ...
  void _updateVideoQuality() {
    ...
    if (newQuality != _currentVideoQuality) {
      ...
      print('🎥 Video quality changed to: ${newQuality.resolution} @ ${newQuality.bitrate}kbps');
    }
  }
  ...
  int getRecommendedPreloadCount() {
    ...
    return switch (_currentNetworkQuality) {
      NetworkQuality.excellent => 5, // Was 3
      NetworkQuality.good => 4,      // Was 2
      NetworkQuality.fair => 3,      // Was 1
      NetworkQuality.poor => 2,      // Was 0 - even poor networks should preload
    };
  }
