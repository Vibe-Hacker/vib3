...
class BufferManagementService {
  ...
  static const int _maxActiveControllers = 3;  // Maximum simultaneous video controllers
  ...
  void initialize() {
    ...
    print('🧹 BufferManagementService initialized');
  }
  
  void dispose() {
    _bufferCleanupTimer?.cancel();
    _activeControllers.clear();
    print('🧹 BufferManagementService disposed');
  }
  ...
  bool registerController(VideoPlayerController controller) {
    ...
    print('📱 Registered controller. Active count: ${_activeControllers.length}');
    return true;
  }
  ...
  void unregisterController(VideoPlayerController controller) {
    ...
    print('📱 Unregistered controller. Active count: ${_activeControllers.length}');
  }
  ...
  void _cleanupOldestControllers() {
    ...
    for (final controller in controllersToRemove) {
      try {
        ...
        controller.dispose();
        _activeControllers.remove(controller);
        print('🧹 Cleaned up inactive controller');
      } catch (e) {
  ...
  void _performBufferCleanup() {
    ...
    if (controllersToRemove.isNotEmpty) {
      print('🧹 Cleaned up ${controllersToRemove.length} invalid controllers');
    }
  }
  
  // Emergency cleanup - dispose all controllers
  void emergencyCleanup() {
    print('🚨 Emergency buffer cleanup initiated');
    ...
    for (final controller in _activeControllers.toList()) {
      try {
        ...
        print('⚠️ Error in emergency cleanup: $e');
      }
    }
