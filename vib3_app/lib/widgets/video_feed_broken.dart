...
class _VideoFeedState extends State<VideoFeed> with WidgetsBindingObserver {
  ...
  void _onPageChanged(int index) {
    ...
    Future.delayed(const Duration(milliseconds: 50), () {
      ...
      // Get the appropriate videos list based on feed type
      List<Video> videos = [];
      if (widget.feedType == FeedType.forYou) {
        ...
        videos = videoProvider.discoverVideos;
      } else {
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: GestureDetector(
            ...
            onLongPressMoveUpdate: (details) {
              if (_draggingButton == buttonId && _initialDragPosition != null && mounted) {
                setState(() {
  ...
  Widget build(BuildContext context) {
    return Consumer<VideoProvider>(
      builder: (context, videoProvider, child) {
        // Select videos based on feed type with consistent fallback
        List<Video> videos = [];
        if (widget.feedType == FeedType.forYou) {
        ...
        // Prevent video switching by maintaining stable list when loading
        if (videoProvider.isLoading && videos.isEmpty) {
          videos = []; // Keep empty while loading to prevent switches
        }

        // Handle initial video if provided
        if (widget.initialVideo != null && !videos.contains(widget.initialVideo)) {
          videos = [widget.initialVideo!, ...videos];
        }

        // Debug: Print video count
        print('VideoFeed: Displaying ${videos.length} videos, isLoading: ${videoProvider.isLoading}');
        ...
        // Check for empty videos after loading is complete
        if (!videoProvider.isLoading && videos.isEmpty) {
          return Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    final token = authProvider.authToken;
                    if (token != null) {
                      // Retry loading videos for the current feed type
                      if (widget.feedType == FeedType.forYou) {
        ...
        return PageView.builder(
            ...
            itemCount: videos.length + (videoProvider.hasMoreVideos ? 1 : 0),
            ...
            return Container(
              ...
              child: Center(
                child: Container(
                  width: kIsWeb ? 600 : MediaQuery.of(context).size.width * 0.95, // Fixed width on web
                  ...
                  child: ClipRRect(
                    ...
                    child: Stack(
                      children: [
                        ...
                        // Video description overlay (centered with video)
                        Positioned(
                          bottom: 60,
                          left: 0,
                          right: 0,
                          child: Center(
                          child: Container(
                            width: kIsWeb ? 600 : MediaQuery.of(context).size.width * 0.95, // Same width as video container
                      ...
                      ...(_isDragMode || _buttonPositions.isNotEmpty 
...
class _CommentsSheetState extends State<CommentsSheet> {
  ...
  Future<void> _loadComments() async {
    ...
    try {
      final comments = await CommentService.getVideoComments(widget.video.id, token);
      if (mounted) {
        setState(() {
          _comments.clear();
  ...
  Future<void> _likeComment(Comment comment) async {
    ...
    final success = await CommentService.likeComment(comment.id, token);
    
    if (success) {
      // Update the comment's like status locally
                                        ...
                                        child: Row(
                                          children: [
                                            Icon(
                                              ...
                                              color: comment.isLiked ? const Color(0xFFFF0080) : Colors.grey,
                                              size: 16,
                                            ),
              ...
              children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    padding: const EdgeInsets.all(8),
                    decoration: BoxDecoration(
                      color: _isPosting ? Colors.grey : const Color(0xFFFF0080),
                      shape: BoxShape.circle,
                    ),
                    child: _isPosting
                        ? const SizedBox(
