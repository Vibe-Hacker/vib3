...
async function changeProfilePicture() {
    ...
    modal.innerHTML = `
        <div style="background: #222; padding: 30px; border-radius: 12px; max-width: 400px; width: 90%;">
            ...
            <div style="background: #333; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                ...
                <input type="file" id="profileImageInput" accept="image/*" style="margin-bottom: 15px;">
                <button onclick="uploadProfileImage()" style="background: #fe2c55; color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer; width: 100%;">
                    Upload Image
                </button>
            </div>
            ...
            <div style="background: #333; padding: 20px; border-radius: 8px;">
                <h4 style="color: #fe2c55; margin-bottom: 15px;">😀 Choose Emoji</h4>
                <div style="display: grid; grid-template-columns: repeat(6, 1fr); gap: 10px; margin-bottom: 15px;">
                    ${emojis.map(emoji => `
                        <button onclick="selectProfilePicture('${emoji}')" 
                                style="background: ${emoji === currentEmoji ? '#fe2c55' : '#444'}; color: white; border: none; padding: 15px; border-radius: 8px; cursor: pointer; font-size: 20px;">
                            ${emoji}
                        </button>
                    `).join('')}
                </div>
            ...
            <div style="display: flex; gap: 10px; margin-top: 20px;">
                <button onclick="closePictureModal()" style="flex: 1; background: #333; color: white; border: none; padding: 12px; border-radius: 6px; cursor: pointer;">
                    Cancel
                </button>
            </div>
        </div>
    ...
    modal.onclick = (e) => {
        if (e.target === modal) closePictureModal();
    };
    ...
    window.uploadProfileImage = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.error('❌ Profile image upload error:', response.status, errorText);
